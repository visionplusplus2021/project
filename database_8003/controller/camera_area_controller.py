...
def camera_area_get():
    ...
    result = conn.selectData('select * from vw_area_all')
    return result
...
def camera_area_get_active():
    ...
    result = conn.selectData('select * from vw_camera_area_active')
    return result
...
def camera_area_getByID(val):
    ...
    result = conn.selectData('''select * from vw_area_all where "AreaID" = '{0}' '''.format(val))
    return result
