...
def camera_group_get():
    ...
    result = conn.selectData('select * from vw_camera_group_all')
    return result
...
def camera_group_getByID(val):
    ...
    result = conn.selectData('''select * from vw_camera_group_all where "CameraGroupID" = '{0}' '''.format(val))
    return result
...
def camera_group_get_activate(val):
    ...
    result = conn.selectData('''select * from vw_camera_group_active where "CameraGroupType" = {0} '''.format(val))
    return result
