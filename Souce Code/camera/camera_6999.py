...
def checkNotification():
    threading.Timer(5.0, checkNotification).start()
    
    
    

    docs = requests.get(camera.database+'/customer/get_period')
    ...
    ### add email
    for data in j_data:
        if(contact_id != data[9]):
            email.append([data[7],data[9],str(data[10])+"-"+str(data[11])+"-"+str(data[12])])
            contact_id = data[9]
    ...
    for e in email:
        ...
        for data in j_data:
            ...
            if(e[1] == data[9]):
                jay_data.append(data)


    if(len(jay_data) > 0):
        
        
        noti.send_notification_byemail(e,"Jaywalking "+str(len(jay_data)),jay_data,camera.database)
        requests.post(camera.database+"/customer/update_notification/"+e[1])
        # print("this is the new template ==+++> my data "+str(jay_data))

        for t in  last_send:
            diff_time = datetime.now() - t[1]
            hh, mm, ss = str(diff_time).split(':')
            second = str(ss).split('.')
            diff_second = int(hh) * 3600 + int(mm) * 60 + int(second[0])

            print('Time in seconds:'+ str(diff_second))

            print("time => "+str(diff_time)+ " === > t = "+str(t))     
            
        last_send.append([e[1],datetime.now()])    
        print("============================")       
