...
$(document).ready(function () {
  ...
  $('#buttonServerAdd').click(function () {
    ...
    if (serverName == '') {
        ...
        if (request.readyState === 4) {
          if (request.status === 200) {
            ...
            $.ajax({
              type: 'GET',
              dataType: 'json',
              url: "http://199.212.33.166:9905/run_server_port/"+serverPort,
      ...
      request.open('GET', "http://199.212.33.166:9905/test_camera_connection/"+serverPort, true);
      request.send(params);
...
function getServerInfo() {
  ...
  $.ajax({
    type: 'GET',
    url: "http://"+inputServerIP+":9905/get_server_info",
    success: function (data) {
      str_data = data.split("[");
