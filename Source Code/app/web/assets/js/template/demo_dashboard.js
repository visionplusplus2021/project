$(document).ready(function () {
  ...
  $('#buttonDeleteCamera').click(function () {
    ...
    $.ajax({
      type: 'DELETE',
      url: database_url + "/video_demo/delete/" + camID,
      success: function (data) {
        location.reload();
      }
  ...
  $('#ModalUpdateCamera').on('show.bs.modal', function (e) {
    ...
    $.ajax({
      type: 'GET',
      url: database_url + "/video_demo/getByID/" + objectID,
      success: function (data) {
  ...
  $('#ModalPresetCounting').on('show.bs.modal', function (e) {
    ...
    $("#containerRowTest").html(`<div id="containerRowTest"><div><img class="card-img-top" src="img/video/${cameraID}.jpg" id="img_counting"
  ...
  $('#saveInfo').on('click', function (e) {
    ...
    if (cameraName && cameraURL && cameraGroup ) {
      ...
      // server_id = server_data[0]
       server_ip_port = "199.212.33.166:8899"
              ...
              success: function (list) {
                ...
                fetch(database_url + '/demo/store', {
                  ...
                  .then(response => {
                    ...
                    $.ajax({
                      type: 'GET',
                      url: database_url + "/video/getByName/" + cameraName + "_" + cameraStream,
                      success: function (data) {
      ...
      request.open('GET', "http://199.212.33.166:8899/test_camera_connection/"+cameraStream+"_"+uuid, true);
  ...
  function getBoundary(count_type)
  {
    ...
    $.ajax({
      type: 'GET',
      url: "http://199.212.33.166:8899/draw_boundary_byID/" + cameraID+"?date_time="+dt+"&feature_type="+count_type,
      success: function (data) {
           ...
           $("#containerRowTest").empty();
           $("#containerRowTest").html(`<div id="containerRowTest"><div><img class="card-img-top" src="img/video/${cameraID}_${dt}.jpg" id="img_counting" name= 'img_counting'></div></div>`);
  ...
  function getUpdateFeatures(features, objectID, serverIP) {
    ...
    $.ajax({
      type: 'GET',
      url: database_url + "/video_demo_feature/getByID/" + objectID,
      success: function (data) {
        var temp = [];
  ...
  function renderCards(isValid) {
    $.ajax({
      type: 'GET',
      url: database_url + "/camera_group/get_activate/1",
      success: function (data) {
        ...
        $.ajax({
          ...
          success: function (data) {
            JSON.parse(data).forEach((results, index) => {
              suspendd_flag = "false"
              server_port = "199.212.33.166:8899" 
              ...
              if (results[6] == true) {
                $("#containerRow").html(`<div id="containerRow"><div><h4>${results[1]} </h4><img id="mainCam" src='http://${server_port}/stream/${results[0]}_${results[4]}'  class="img-fluid w-100 h-auto m-0 p-0 ${str_img}"; ></div></div>`);
              }
              ...
              $('#' + results[2]).append(
                `<div class="card" id="c${results[0]}" style="width: 18rem;">
                     ...
                     <img src="img/demo_file/${results[4]}.jpg" id="img${results[0]}">
                      <div id="cameras" class="card-body">
                        <h5 class="card-title">${results[1]}</h5>
                        <p class="card-text"></p>

                        <a id="edit${results[0]}" class="isAdmin btn btn-primary  btn-sm" data-toggle="modal" data-target="#ModalUpdateCamera" data-id="${results[0]}">Edit</a>
                        <a id="del${results[0]}" class="isAdmin btn btn-danger  btn-sm" data-toggle="modal" data-target="#ModalDeleteCamera" data-id="${results[0]}_${server_port}">Delete</a>
              ...
              $('#img' + results[0]).click(function () {
                ...
                $("#containerRow").html(`  <div id="containerRow"><div><h4>${results[1]}</h4><img id="mainCam" src='http://${server_port}/stream/${results[0]}_${results[4]}' class="img-fluid w-100 h-auto m-0 p-0"></div></div>`);
                
                var database_endpoint = database_url + '/video_demo/setActive';
