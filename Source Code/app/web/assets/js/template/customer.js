...
$(document).ready(function () {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        JSON.parse(data).forEach((results, index) => {
          ...
          var bool_check = true;
          for ( i=0; i < add_feature.length;i++)
          {
            if (add_feature[i] == results[0] )
            {
              bool_check = false;
              break;
            }
  ...
  $('#btnAddContact').click(function () {
    ...
    // var notitype = $('input:radio[name="myRadios"]').val();
