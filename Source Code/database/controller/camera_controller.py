...
def camera_feature_getByID(val):
    ...
    if(val=="1"):
        result = conn.selectData('''select * from visionplusplus.vw_camera_feature_add''')
    else:
        result = conn.selectData('''select * from visionplusplus.vw_camera_feature  where "CameraID" = '{0}' or "CameraFeatureID" is null  '''.format(val))
    return result
...
def camera_feature_getByName(val):
    ...
    result = conn.selectData('''select * from visionplusplus.vw_camera_detail_all  where "CameraName" = '{0}' '''.format(str_data[0]))
...
@app.route('/camera/get_jaywalking_camera', methods=['GET'])
def get_jaywalking_camera():

    ## Check set default
    
    
    result = conn.selectData('''select * from visionplusplus.vw_jaywalking_camera  ''')
    return result
...
def camera_getBoundaryByID(val):
    ...
    result = conn.selectData('''select * from visionplusplus.vw_camera_feature_display  where "CameraID" = '{0}' '''.format(val))
    return result

@app.route('/camera/getBoundaryByCameraFeatureID/<val>', methods=['GET'])
def camera_getBoundaryByFeatureID(val):

    result = conn.selectData('''select * from visionplusplus.vw_camera_feature_display  where "CameraFeatureID" = '{0}' '''.format(val))
    return result
    
@app.route('/camera/getCamerabyStreamID/<val>', methods=['GET'])
def camera_getCamerabyStreamID(val):

    result = conn.selectData('''select * from visionplusplus.vw_camera_stream_active  where "CameraStreamID" = '{0}' '''.format(val))
    return result
