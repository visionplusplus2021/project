...
def jaywalking_report_get_by_filter():
    ...
    sql = '''select "BoundaryName","ClassName", count(*)   from vw_sum_object_counting   where "BoundaryType" = 'jaywalking' and  "TimeStamp" between '{0}' and '{1}'  
                                 {2} {3} group by "BoundaryName","ClassName"  order by "BoundaryName"  '''.format(start_date,end_date,str_lane_name,str_camera_name)
