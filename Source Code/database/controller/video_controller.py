...
def video_get():
    ...
    result = conn.selectData('''select * from visionplusplus.vw_video_demo_detail_all  ''')
...
def video_demo_getFileID(val):
    ...
    result = conn.selectData('''select * from visionplusplus.vw_video_demo_detail_all  where "VideoDemoFileID" = '{0}' '''.format(val))
    return result
...
def video_feature_getByName(val):
    ...
    result = conn.selectData('''select * from visionplusplus.vw_video_demo_detail_all  where "VideoDemoName" = '{0}' '''.format(str_data[0]))
...
def video_demo_feature_getByID(val):
    ...
    if(val=="1"):
        ...
        result = conn.selectData('''select * from visionplusplus.vw_video_demo_feature  where "VideoDemoID" = '{0}' or "VideoDemoFeatureID" is null  '''.format(val))
    return result
