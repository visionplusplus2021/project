...
def feature_get():
    ...
    result = conn.selectData('select * from visionplusplus.vw_feature_all')
    return result


@app.route('/feature/get_isActivate', methods=['GET','POST'])
def feature_get_isActivate():
    result = conn.selectData('select * from visionplusplus.vw_feature_active')
    return result
...
def feature_getByID(val):
    ...
    result = conn.selectData('''select * from visionplusplus.vw_feature_all where "FeatureID" = '{0}' '''.format(val))
    return result
