...
def camera_getVehicleCountingByCameraID(val):
    ...
    result = conn.selectData('''select * from visionplusplus.vw_counting_vehicle  where "CameraID" = '{0}' '''.format(val))
    return result
...
def counting_boundary_get(val):
    ...
    result = conn.selectData('''select * from visionplusplus.vw_camera_feature_boundary where "CameraFeatureID" = '{0}' '''.format(val))
    return result
...
def counting_boundary_get_lane_vehicle(val):
    ...
    result = conn.selectData('''select * from visionplusplus.vw_camera_feature_boundary where "CameraID" = '{0}'  and "BoundaryType" = '{1}' '''.format(str_data[0],str_data[1]))
    return result
