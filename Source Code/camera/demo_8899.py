...
def yieldMainFrame(data):

    str_data = data.split("_")
    id = str_data[0]
    video_id = str_data[1]
    ...
    # camera.stream_address = camera.getCameraInformation(id)
    # camera.stream = cv2.VideoCapture("./data/video/"+video_id+".mp4")
...
app.config["THUMBNAIL"] = "/media/iotlab/Work/master_vision/app/web/assets/img/video/"
...
def upload_file():
   if request.method == 'POST':
      ...
      f.save(demo_path+"/"+secure_filename(f.filename))
