...
from core.notification import Notification as noti
...
class Video:
        ...
        for i in range(len(j_data)):
            try:
                for j in range(len(j_data[i]["features"])):
                    ...
                    if("Smoke/Fire Detection" in j_data[i]["features"][j]):
                        str_feature[1] = True
                    ...
                    if("Laneway Segmentation" in j_data[i]["features"][j]):
                        str_feature[4] = True

                    if("Jaywalking" in j_data[i]["features"][j]):
                        str_feature[5] = True
    ...
    def updateProcessedRawStream(self):
        ...
        # self.checkFeature(self.cam_id_test)
        ...
        while True:
            ...
            #print("========self.cam_id_test===="+str(self.cam_id_test)+"=======>"+str(str_features))
            ...
            # if control_check['counting_check']==True:
                ...
                for track in tracks:
                    ...
                    requests.post('http://127.0.0.1:8003/count/store', data = object_)
                    ...
                    if c >= 3:
                        ...
                        if not kcw.recording:
                            ...
                            database_url_events = 'http://127.0.0.1:8003/events/create'
                            requests.post(database_url_events, data = object_event)
                            ##############
            ...
            #if(str_features[0]):
            if(True):
                frame,bool_detected = Trespassing.person_detection(frame, bboxes, scores, names,self.ip)
               
            if(str_features[5]):
                frame,bool_detected = Trespassing.jwalking_detection(frame, bboxes, scores, names,self.ip)

                if(bool_detected):

                    if(self.jwalking_count < -5 ):
                        self.jwalking_count = 0

                    if(self.out.isOpened() == False and self.jwalking_count==0):
                        fourcc = cv2.VideoWriter_fourcc(*'XVID')
                        timestamp = datetime.now()
                        h, w, c = frame.shape
                        video_name = timestamp.strftime("%Y%m%d-%H%M%S")+".avi"
                        self.out = cv2.VideoWriter("events/jaywalking/"+video_name,fourcc, 3.0, (w, h))
                        #noti.send_notification_email(video_name,frame)
                        #noti.send_SMS(video_name,frame)
