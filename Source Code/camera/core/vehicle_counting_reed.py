    ...
    def vehicle_couting_detection(self,frame,database,lanes,boxes,classes,class_indexs,confidences,previous_frame,deeps):
        ...
        for lane in lanes:
            ...
            for track in tracks:
                ...
                W = int((track_bbox[2] -track_bbox[0] )/2 )
                H = int((track_bbox[3] -track_bbox[1])/2)

                X = int(track_bbox[0]) + +int(W/2)
                Y = int(track_bbox[1]) + +int(H/2)
                ...
                if(track_class != "person"):
                    point = point_shape(X+W, Y+H-20)
                    cv2.circle(frame,(X+W, Y+H-20), 1, (0, 150,0), 5)
                else:
                    point = point_shape(X+int(W/2), Y+H-10)
                    cv2.circle(frame,(X+int(W/2), Y+H-10), 5, (0, 150,0), 5)
